...
import { ArrowLeft, Check, Copy, Cursor, GithubLogo, LinkSimple, QrCode as QrCodeIcon, UsersThree, XLogo } from '@phosphor-icons/react';
...
export function ProfilePage() {
  ...
  const shareText = useMemo(() => {
    ...
    const x = attendee.x_handle ? ` Follow @${attendee.x_handle} to find me online.` : '';
    return `I'm the ${colorName(attendee.cursor_color)} ${attendee.cursor_code} cursor at Cursor Roadshow Bangalore. Building: ${attendee.project || 'something interesting'}${x} Find me: ${zone}${clue} Come say hi: ${profileUrl}`;
  }, [attendee, profileUrl]);
  ...
  async function meet() {
    ...
    try {
      const result = await connectAttendees(session.id, session.token, attendee.id);
      setNotice(result.created ? 'Met IRL. Nice one.' : 'Already connected — still counts.');
    } catch (caught) {
      setNotice(caught instanceof ApiError ? caught.message : 'Could not record that connection.');
    } finally { setConnecting(false); }
  }

  async function copyLink() {
    if (navigator.clipboard?.writeText) { await navigator.clipboard.writeText(profileUrl); setNotice('Profile link copied.'); }
    else setNotice('Automatic copy is unavailable here — copy the address from your browser.');
  }
  ...
  return <section className="mx-auto max-w-[1400px] px-4 pb-20 pt-10 sm:px-8 sm:pt-16">
    ...
    <div className="grid gap-12 lg:grid-cols-[1.12fr_.88fr] lg:gap-20">
      <div>
        <div className="find-card relative overflow-hidden border border-[var(--ink)] bg-[var(--ink)] text-[var(--paper)]">
          <div className="absolute -right-2 top-6 opacity-95 sm:right-8 sm:top-8"><DitherprintAvatar profileId={attendee.id} variant={attendee.avatar_variant} size={112} rounded /></div>
          <div className="relative max-w-2xl p-6 sm:p-10">
            <div className="flex flex-wrap items-center gap-2"><span className="font-mono text-[10px] uppercase tracking-[0.14em] text-white/45">cursor {attendee.cursor_code}</span>{attendee.venue_zone ? <span className="font-mono text-[10px] uppercase tracking-[0.14em] text-white/45">{attendee.venue_zone}</span> : null}</div>
            <h1 className="mt-20 max-w-[620px] text-[clamp(3.5rem,9vw,7.8rem)] font-semibold leading-[.86] tracking-[-0.09em]">{attendee.name}<span className="text-[var(--accent)]">.</span></h1>
            <p className="mt-7 max-w-xl text-xl leading-snug text-white/70">{attendee.project || 'Building in public.'}</p>
          </div>
          <div className="grid gap-0 border-t border-white/15 sm:grid-cols-2">
            <Info label="Find me" value={attendee.outfit_clue || 'Ask for this cursor.'} className="sm:border-b sm:border-r" />
            <Info label="Zone" value={attendee.venue_zone || 'Somewhere in the room'} className="sm:border-b" />
            <Info label="Looking for" value={attendee.looking_for || 'A good conversation'} className="sm:border-b-0 sm:border-r" />
            <Info label="Handles" className="sm:border-b-0"><div className="mt-3 flex flex-wrap gap-x-4 gap-y-2"><HandleLink href={attendee.x_handle ? `https://x.com/${attendee.x_handle}` : undefined} label={attendee.x_handle ? `@${attendee.x_handle}` : undefined}><XLogo size={14} /></HandleLink><HandleLink href={attendee.cursor_handle ? `https://cursor.com/@${attendee.cursor_handle}` : undefined} label={attendee.cursor_handle ? `@${attendee.cursor_handle}` : undefined}><Cursor size={14} /></HandleLink><HandleLink href={attendee.github_handle ? `https://github.com/${attendee.github_handle}` : undefined} label={attendee.github_handle}><GithubLogo size={14} /></HandleLink>{!attendee.x_handle && !attendee.cursor_handle && !attendee.github_handle ? <span className="text-white/45">No public handles</span> : null}</div></Info>
          </div>
        </div>
        <div className="mt-5 flex flex-wrap gap-3">{session?.id === attendee.id ? <span className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full border border-[var(--line-strong)] bg-white/45 px-5 text-sm font-semibold text-[var(--muted)]"><Check size={16} weight="bold" className="text-[#5C9C77]" /> This is you</span> : <Button onClick={() => void meet()} loading={connecting} className="flex-1"><UsersThree size={17} /> I met this person</Button>}<Button variant="ghost" onClick={() => void copyLink()} aria-label="Copy profile link"><Copy size={17} /> Copy link</Button></div>
        {notice ? <div className="mt-4 flex items-center gap-2 text-sm text-[var(--muted)]"><Check size={16} weight="bold" className="text-[#5C9C77]" />{notice}</div> : null}
        <CursorUsage handle={attendee.cursor_handle} />
      </div>
      <aside className="space-y-8 lg:pt-10">
        <div className="border-t border-[var(--line-strong)] pt-5"><div className="flex items-center justify-between"><div><p className="font-mono text-[10px] uppercase tracking-[0.14em] text-[var(--muted)]">Scan to find them</p><h2 className="mt-2 text-2xl font-semibold tracking-[-0.05em]">Take this card with you.</h2></div><QrCodeIcon size={22} className="text-[var(--muted)]" /></div><div className="mt-6 flex flex-col items-center gap-5 border-y border-[var(--line-strong)] bg-[var(--sand)] px-5 py-7"><QrCode value={profileUrl} size={180} /><p className="max-w-[220px] text-center text-xs leading-relaxed text-[var(--muted)]">Point your camera here. The person behind <span className="font-mono">{attendee.cursor_code}</span> is one scan away.</p></div></div>
        <div className="border-t border-[var(--line-strong)] pt-5"><p className="font-mono text-[10px] uppercase tracking-[0.14em] text-[var(--muted)]">Share card</p><div className="mt-4"><ShareCard attendee={attendee} url={profileUrl} shareText={shareText} /></div></div>
        <div className="border-t border-[var(--line-strong)] pt-5"><div className="flex items-center justify-between"><div><p className="font-mono text-[10px] uppercase tracking-[0.14em] text-[var(--muted)]">Met IRL</p><h2 className="mt-2 text-2xl font-semibold tracking-[-0.05em]">{attendee.connection_count} {attendee.connection_count === 1 ? 'connection' : 'connections'}</h2></div><LinkSimple size={22} className="text-[var(--muted)]" /></div><div className="mt-4"><ConnectionList connections={attendee.connections} /></div></div>
      </aside>
    </div>
  </section>;
}

function HandleLink({ href, label, children }: { href?: string; label?: string | null; children: ReactNode }) {
  return href && label ? <a href={href} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 font-semibold transition hover:text-white">{children} {label}</a> : null;
}
